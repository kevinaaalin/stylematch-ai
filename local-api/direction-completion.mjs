...
export function validateDirectionCompletion(capture) {
  if (capture?.input_mode !== "partial_direction_completion") throw new Error("Invalid direction completion mode");
  if (capture.shared_center_confirmed !== true) throw new Error("請確認照片屬於同一空間與共同拍攝中心。");
  const views = capture.ordered_sources;
  if (!Array.isArray(views) || views.length < 1 || views.length > 4) throw new Error("請提供 1 至 4 個已知方向。");
