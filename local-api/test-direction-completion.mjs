...
const concept = { ...capture, input_mode: "concept_direction_completion", ordered_sources: [], concept_only_confirmed: true };
assert.equal(validateDirectionCompletion(concept).length, 0);
assert.throws(() => validateDirectionCompletion({ ...concept, concept_only_confirmed: false }));
assert.throws(() => validateDirectionCompletion({ ...concept, ordered_sources: capture.ordered_sources }));
