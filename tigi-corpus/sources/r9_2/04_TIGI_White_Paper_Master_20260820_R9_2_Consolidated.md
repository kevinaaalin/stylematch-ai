...
R9.2 Consolidated Technical Baseline Governance Integrated｜四母本完整內容整合最新版
...
章節結構：共 21 章；R6.1 治理整合版獨立保存，不覆寫 R5.2／R5.2.1
...
本附錄將 TIGI White Paper Master 納入 TIGI Governance Master R6.1 的共同治理基線。
...
白皮書母本｜版本 20260810_R7_2_Style_Proposal_Vision_Commercial_QA_Integrated｜發布識別 TIGI-GOVERNANCE-20260810-R7.2-SPVC-QA
...
TIGI White Paper Master｜20260810_R7_2_Style_Proposal_Vision_Commercial_QA_Integrated｜TIGI-GOVERNANCE-20260810-R7.2-SPVC-QA
...
TIGI White Paper Master｜20260813_R8_StyleMatch_iSAFE_Integrated｜TIGI-GOVERNANCE-20260813-R8-SM-ISAFE
...
TIGI White Paper Master｜20260814_R9_Patent_V7_Governance_Aligned｜TIGI-GOVERNANCE-20260814-R9-PATENT-V7
...
- 白皮書以可追溯、可解釋、人機分工與正式治理權限分離為核心，不把 Patent V7 對齊描述成法律上的專利有效性意見。
- 外部 AI、Rule Engine、通知與工具呼叫均是治理輸入；正式效力仍由 iSAFE、Decision Object、Gate、Authority 與 R5.2 State Machine 形成。
- 產業倡議應揭露願景、規格、本地實作、治理核准與 production deployment 的成熟度差異。
...
R9 建立平台、Agent 與治理骨架；R9.1 進一步把空間本身轉成可理解、可校正、可版本化、可交換、可核准的設計智能資料。這不是以 AI 取代設計專業，而是建立人機協作的共同語言。

| 過去 | R9.1 | 公共價值 |
| --- | --- | --- |
| 平面圖 → AI 看懂 → 生圖 | 平面圖 → StructuredSpace → 配置／視覺／材料／提案／治理 | 減少語意斷裂與資訊重工 |
| 成果是單張圖片 | 成果是版本化空間、資產與核准快照 | 可追溯、可比較、可驗收 |
| 工具各自封閉 | Canonical Object＋Adapter＋Handoff | 提高跨平台互通與資料主權 |


R9.1 SPATIAL DESIGN INTELLIGENCE

## StructuredSpace：共同空間語言

產業標準、空間資料治理、人機協作、可信 AI 與生態系願景

StructuredSpace 是 R9.1 的核心 Canonical Object，承接 rooms、walls、openings、dimensions、fixtures、furniture、zones 與 circulation_graph，並以 confidence、correction refs、schema version、checksum 與 approval state 管理不確定性。

| 原則 | 意義 |
| --- | --- |
| 可校正 | AI 辨識不是最終真相；專業者可修正並建立新 revision |
| 可版本化 | 提案、配置、視圖與材料必須指向明確 space snapshot |
| 可交換 | 外部工具透過 adapter 映射，不強迫單一軟體成為唯一標準 |
| 可治理 | 核准與移交帶 checksum、來源、假設、風險與 evidence manifest |
| 概念／工程分級 | 缺比例尺或尺寸時標示 conceptual，不冒充施工模型 |
...
| 能力 | 適合 AI／生成式方法 | 必須確定性／人工判斷 |
| --- | --- | --- |
| 空間理解 | 辨識與候選語意 | 比例、尺寸、校正、核准 |
| Auto Layout | 提出候選、偏好排序 | 碰撞、開門、淨距、動線與法規 |
| 視覺設計 | 風格、材質、構圖與候選圖 | 需求符合、可建造性與最終採用 |
| 360 一致性 | 多視角生成與重試 | anchor 選擇與 Approved ViewSet |
| 材料／產品 | 搜尋、風格與預算匹配 | 價格、供貨、規格與採購確認 |

- 可信 AI 的重點不是消除所有不確定性，而是讓不確定性被標記、量測、校正與負責。
- 模型能力與工程規則互補：生成負責探索，規則負責邊界，人負責價值、責任與核准。

R9.1 SPATIAL DESIGN INTELLIGENCE

## Human Authority 與治理護欄

產業標準、空間資料治理、人機協作、可信 AI 與生態系願景

| 治理護欄 | R9.1 原則 |
| --- | --- |
| Human Authority | Candidate Artifact 不得自動成為 Approved Artifact 或正式治理決策 |
| State Authority | 只有 iSAFE R5.2 Gate／Authority 可改正式案件狀態 |
| Traceability | Prompt、input refs、provider、版本、policy、output、成本與 approval 可回溯 |
| Tenant／Role | 所有 job、asset、catalog、connector 與 handoff 受租戶、專案、角色隔離 |
| Payment Separation | Gate PASS、Payment Eligibility、Payment Approval、Invoice、Execution 分離 |
| Knowledge Trust | 候選知識、可信知識與 Knowledge Gap 不混用 |
...
| 角色 | 責任 | 交換物 |
| --- | --- | --- |
| StyleMatch AI | 需求、空間、提案與設計智能 | Approved Proposal／Space／Assets／Handoff |
| Agent Platform | 規劃、工具、政策與 trace | Candidate artifacts／approval request／Handoff V2 |
| TWCID | 會員、媒合、認證與市場生態 | 身分／資格／媒合參照 |
| iSAFE 2.0 | 正式治理決策與履約監管 | Gate、Evidence、Decision、Audit、Payment Eligibility |
| 外部設計工具 | 專業編輯與場景產出 | Scene／Camera／Viewport／Canonical refs |
| 材料／產品夥伴 | 可驗證型錄與供應資訊 | 版本化 catalog／price／availability observation |

- 互通不等於資料無限制集中；最小必要交換、來源透明、版本明確與使用者授權是資料主權基礎。
- Canonical schema 應開放治理規格，但正式 registry、authority 與 release 必須受版本控制。
...
**版本決議。** 本 TIGI White Paper Master 為 TIGI R9.2 的同版發布文件，完整保留 R9.1 內容並統整版本治理；文件升版不代表 Candidate Implementation 已完成程式實作。
