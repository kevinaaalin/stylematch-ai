...
import { approveAsset, approveAutoLayout, approveLocalArtifact, approveStructuredSpace, buildGovernanceHandoffV2, correctStructuredSpace, createApprovedAsset, createCaseCreationProposal, createLocalArtifact, createStructuredSpace, decideCaseCreationProposal, executeCaseCreationProposal, generateAutoLayoutCandidates, listApprovedAssets, listAutoLayouts, listGovernanceHandoffsV2, listLocalArtifacts, listStructuredSpaces, parseFloorplan, receiveGovernanceHandoffV2, validateAutoLayout } from "@/lib/structuredSpaceApi";
...
export default function StructuredSpacePanel({ projectId, floorPlanUrl }) {
  ...
  const refresh = async () => {
    ...
    setBusy("load");
    try {
      const [spaceResult, layoutResult, handoffResult, assetResult, viewsetResult, materialResult, sceneResult] = await Promise.all([listStructuredSpaces(projectId), listAutoLayouts(projectId), listGovernanceHandoffsV2(projectId), listApprovedAssets(projectId), listLocalArtifacts(projectId, "viewset"), listLocalArtifacts(projectId, "material_selection"), listLocalArtifacts(projectId, "external_scene")]);
      setSnapshots(spaceResult.snapshots || []); setLayouts(layoutResult.layouts || []); setHandoffs(handoffResult.handoffs || []); setAssets(assetResult.assets || []); setLocalArtifacts([...(viewsetResult.artifacts || []), ...(materialResult.artifacts || []), ...(sceneResult.artifacts || [])]); setMessage("");
    }
  ...
  const parseUploadedFloorplan = async () => {
    ...
    try {
      const parsed = await parseFloorplan(projectId, {
        ...
        hints: { primary_room_name: roomName || "待確認空間", units: "mm" },
      });
      setMessage(`離線解析已建立版本 ${parsed.revision}，信心 ${Math.round(parsed.confidence * 100)}%；請人工校正後再核准。`);
      await refresh();
    } catch (error) { setMessage(error.message); }
    finally { setBusy(""); }
  };
